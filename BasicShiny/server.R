...
shinyServer(function(input, output) {
  ...
  observeEvent(input$buttonId, {
      message("running Proteomics Code.")
    ...
    #make sure startix and endix are the first and last abundance columns of the data frame 
    ...
    # VOLCANO
    p <- ggplot(tt.df,aes(x=logFC,y=-log(P.Value))) + geom_point(size=2, alpha=1, col='black')
    p <- p + labs(title='Diff Expressed Proteins for Control vs Treatment at P Value <= 0.05', x='log(FC)', y='-log(nominalpval)')
    p <- p + theme(plot.title=element_text(size=10, vjust=1, hjust=0.5), legend.position='none')
    
    p <- p + geom_point(data=tt.df, stat='identity', col='red', size=1) + geom_hline(yintercept=2.5) + geom_vline(xintercept=-1) + geom_vline(xintercept=1)
    p <- p + geom_text_repel(data=tt.df[1:30, ], aes(x=logFC, y=-log(P.Value), label=tt.df$symbol[1:30]), colour='forestgreen', size=2)
    
    #change file name
    tiff(paste("FASInformatics/Figures/", input$outputfile, "_Volcano.tiff"), width = 6, height = 4, units = 'in', res = 600)
